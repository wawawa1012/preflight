...
class ReviewStorageTest(unittest.TestCase):
    ...
    # 7. 未绑定 rubric 的 Material 可加入
    def test_upsert_unbound_material_allowed(self) -> None:
        review = self._review()
        material = self._material()
    ...
    # 反向守卫 1：unbound M → 加入 Review A → 首次绑定 B → BindingConflict，保持未绑定且成员仍在
    def test_first_binding_rejected_when_review_uses_different_rubric(self) -> None:
        review = self._review()
        material = self._material()
    ...
    # 反向守卫 2：unbound M → 加入同标准 A/A2 → 首次绑定 A 成功
    ...
    # 反向守卫 3：unbound M → 加入不同标准 A/B → 绑定 A 与 B 都拒绝
    def test_first_binding_rejected_when_two_conflicting_reviews(self) -> None:
