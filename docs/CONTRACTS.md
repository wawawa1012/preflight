...
- PDF page、PPTX slide、DOCX paragraph、MD/TXT line 均从 1 开始；block.ordinal 从 0 开始，正文顺序一律以 Block.ordinal 为准，不用 line_number 排序。
- Locator v1：`kind ∈ {page, slide, paragraph, line, table_cell}`。line/paragraph 时 index 是真实 1 基行号/段落序号；table_cell 时 index 是一基 table 序号，row_index/cell_index/paragraph_index 是表内一基结构序号，block_index 保留既有含义（同一来源单位内的块分组序号）。非行来源的 line_number/line_count 必须为 null，不得用 index 冒充行号；历史 Markdown wire 值不变。
...
- 来源复验统一走 `evidence.resolve_source_ref`：quote-only 保留第一次 occurrence 兼容行为；显式 start/end 必须同时提供并逐字命中，失败 400 span_mismatch，绝不静默退回第一次匹配；显式 span 因此可以选择重复文本的第二次及以后 occurrence。
...
| POST /api/v1/evidence-annotations | {block_id, quote, note?, material_id?, start?, end?} | EvidenceAnnotation，201；quote 未命中 400 quote_not_found、显式 span 不符 400 span_mismatch、归属不符 400 material_mismatch、未知 block 404 block_not_found、start/end 只给一个 400 invalid_request |
...
| POST /api/v1/grill | GrillRequest `{material_id}` | GrillQuestion[]（prompt/quote/block_id/start/end + 程序生成 trigger/preparation）；无来源池零模型调用返回 []；未知 source_id 逐题丢弃；坏响应 502；未配置 503；超时 504 |
| POST /api/v1/response-coach | ResponseCoachRequest `{material_id, question, user_answer, source_refs?, review_id?}` | ResponseCoachResponse（status coached/abstain/insufficient_context + 反馈列表 + 程序回填 sources）；无来源池时无需模型；未知 material 404 material_not_found；review 不存在 404 review_not_found；非成员 400 review_material_mismatch；source_ref 复验失败 400 source_ref_mismatch；LLM 失败 503/502/504/502；不落库 |
...
SavedMaterial 是 2B 的最小持久化实体：materials（id、filename、format、parser_version、size_bytes、sha256、line_count、source_bytes、created_at）与 blocks（id、material_id 外键、ordinal、kind、locator_index、end_index、row_index、cell_index、paragraph_index、text、block_index）两张表，加一个单行 recent_material 指针；不使用 INSERT OR REPLACE。保存与指针更新在同一事务内完成。blocks[].document_id 指向材料 id（当前只有 Material → Block 两级，不是 Document/MaterialVersion，也不是已完成的 Run 或 VersionDiff）。Save 接口服务端重新校验并重新解析上传文件，不信任浏览器回传的 blocks/locator/sha256；原文件字节（source_bytes，md/txt/docx 均保存）与 format、parser_version 跟 material/blocks 同一事务写入。line_count 只对行格式（md/txt）是真实行数，DOCX 为 null；历史 Markdown 材料没有原始字节时 source_bytes=null、parser_version=null，editable-source 继续按 LF/空行规则从 blocks 重建，不伪造原始字节。
Locator v1 迁移：`PRAGMA user_version=1` 把 legacy blocks.line_number 重建为 kind='line' + locator_index，并给 materials 补 format='md'/parser_version=NULL/source_bytes=NULL；Block ID、annotation span、link、binding、revision relation 全部原样保留。迁移可重复执行（已到版本即 no-op）；重建在单事务内完成（外键暂关、提交前 foreign_key_check），失败整体回滚，不留半迁移。
...
EvidenceAnnotation 是 Iteration 3 最小证据层：source 复用冻结 Span（block_id/start/end/quote），服务端用纯函数 resolve_source_ref 校验后才能写入（精确子串、Unicode 代码点索引）。请求体 `EvidenceAnnotationCreate` 可选显式 `start`/`end`（必须同时提供）与可选 `material_id`：quote-only 保留「重复取第一次出现」的历史兼容行为，未命中 400 quote_not_found；显式 span 必须逐字命中 `text[start:end] == quote`，失败 400 span_mismatch、绝不静默退回第一次匹配，因此可以选择同块内第二次及以后的 occurrence；`material_id` 是客户端显式归属断言，与 block 实际所属材料不符时 400 material_mismatch（不提供时由服务端从 block 行派生）。校验与写入在同一事务内完成，库中不存在无效引用。material_id 由服务端从 blocks 行派生，不接受客户端提交；evidence_annotations 对 materials/blocks 双外键（FK CASCADE）。本迭代只表示“引用了真实原文”，尚无 relation/citation_valid，不与 Claim/Finding 关联。
...
Grill（POST /api/v1/grill）是单材料、session-only 的质询官：服务端来源池（findings citations + 前 8 条关键陈述，带局部上下文）经逐字复验后，模型只选 `source_id`；quote/block/坐标由代码回填。Sprint 2 在公开响应中新增三个程序生成字段：`trigger`（来源特征，枚举 numeric_discrepancy/numeric_statement/comparative/absolute/generic）、`why`（「为什么可能被问」的人话映射，不暴露内部机器码）与 `preparation`（3–6 条确定性准备清单，如 numeric_discrepancy → 测试条件/样本量/指标口径/最终采用哪一处），以及程序回填的结构化 `locator`（非行来源不得展示为行）。模型不能提供这些程序字段（额外字段整次 502）；why/清单是准备方向，不是答案、不是结论，也不表示真实评委会问。无来源池不调用模型，返回空数组。
Response Coach（POST /api/v1/response-coach）是 Sprint 2 新增角色：用户已写出回答，教练只检查这条回答能否由给定来源支持；不代写答案、不给分、不评选最佳回答、不判定现实真假、不落库（answer/feedback/会话历史留在前端 session）。请求包含 material_id、question、user_answer、可选 source_refs（≤8；每项 block_id + quote，可选显式 start/end 精确选择 occurrence，统一来源复验失败 400 source_ref_mismatch）与可选 review_id（只带入 Review/rubric/criterion 标题，不带 requirement 正文）。来源池 = 服务端确定性来源（≤12）+ 用户选择来源；模型只输出 source_id，程序回填 quote/block/结构化 locator/span（非行来源 line_number 为 null）。模型 claim `text` 必须是 user_answer 的逐字片段，否则整条丢弃；引用未知 source_id 的 supported claim 降级为 unsupported；unsupported 只表示「当前来源未能支持」，不是现实为假。材料没有可用来源时确定性返回 `insufficient_context` 且不调用模型；模型可返回 `abstain` + reason。错误码：404 material_not_found/review_not_found、400 review_material_mismatch/source_ref_mismatch/invalid_request/material_too_large、503 llm_unconfigured、502 llm_unavailable/llm_invalid_response、504 llm_timeout；Coach 失败不影响已有 Question/Source/preparation。
...
EditableSource / MaterialRevision 的格式边界：`GET /api/v1/materials/{id}/editable-source` 只服务行格式（md/txt），返回 LF 归一化、按 line_count 补空行并保留空行的文本（不承诺 BOM/CRLF/末尾换行）；DOCX 材料请求 editable-source 或创建 revision 一律 400 format_not_editable（不承诺原格式编辑）。`POST /api/v1/materials/{id}/revisions` 的 child 格式由 filename 后缀决定（.md/.txt），text 按 UTF-8 重算 sha256 并重新解析；child 原文件字节（规范化文本 bytes）与 format 一并入库；父材料保持不可变。TXT 与 MD 使用同一真实逐行规则，空行不生成 Block 但保留在 revision/editable-source 中。
...
新增 `AssessmentCreate`、`AssessorProposal`、`AssessmentResult`、`EvaluationScope`、`AssessmentSnapshot`、`AssessmentSummary` 和 `AssessmentComparison`；不修改旧 `CriterionAssessment`/RunReport 语义。评分元数据增量、状态、来源权限、持久化与四个 API 见 [Sprint 4 接入说明](architecture/sprint4-assessment-delivery.md)。结构源仍是 `backend/app/contracts.py`；JSON Schema/TS 从该源生成。无评分定义不产生数字，非 assessed 项不记零，模型不能给总分。可比性同时要求评估器身份一致：`prompt_version` 或 `model_identifier` 与 method/rubric/source policy/材料范围一并比较，任一不同即 `not_comparable` 且不产出逐条 observation（V1 不做跨模型等价判断）。`AssessmentCriterionChange.observation` 精确到维度：`identical` 仅当 status/anchor/score/关键 reason 全等，另有 `anchor_changed`、`reason_changed` 及 `score_changed`/`anchor_changed`/`reason_changed` 布尔供 UI 分别表述。
