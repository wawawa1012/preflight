...
EvidenceAnnotation 是 Iteration 3 最小证据层：source 复用冻结 Span（block_id/start/end/quote），服务端用纯函数 resolve_source_ref 校验后才能写入（精确子串、Unicode 代码点索引）。请求体 `EvidenceAnnotationCreate` 可选显式 `start`/`end`（必须同时提供）：quote-only 保留「重复取第一次出现」的历史兼容行为，未命中 400 quote_not_found；显式 span 必须逐字命中 `text[start:end] == quote`，失败 400 span_mismatch、绝不静默退回第一次匹配，因此可以选择同块内第二次及以后的 occurrence。显式 span 校验与写入在同一事务内完成，库中不存在无效引用。material_id 由服务端从 blocks 行派生，不接受客户端提交；evidence_annotations 对 materials/blocks 双外键（FK CASCADE）。本迭代只表示“引用了真实原文”，尚无 relation/citation_valid，不与 Claim/Finding 关联。
