...
// findings 由服务端给出：这里不重算、不排名、不下结论。
// 结果区是主视觉：每条待核对问题先给人话结论，再左右分栏摊开两份材料的值与原文位置；选材弱化成上下文块。
...
// 空 findings 是检查完成的正常结果：说明选中的两份材料没有可对照的同指标不同数值。
...
// 左右两列的大数字取自该侧引用自带的抽取值：引用是事实源，不从 values 数组猜归属。
function citationValue(citation: ConsistencyFinding['citations'][number]): string {
  return (citation as { value?: string }).value ?? ''
}

function sideValue(finding: ConsistencyFinding, side: 'a' | 'b'): string {
  return sideCitations(finding, side).map(citationValue).find((value) => value !== '') ?? ''
}
...
<template>
  <main class="mx-auto max-w-6xl px-6 py-10">
    <PageHeader title="一致性检查" subtitle="检查两份材料是否对同一指标使用了不同数值。">
    ...
    <!-- 上下文选择器：弱化成一条上下文块；检查动作发出后，主视觉让位给结果区。 -->
    <section class="mt-6">
      ...
      <div v-else-if="loadError" class="rounded-xl bg-slate-950/40 px-5 py-4">
        ...
        <UButton class="mt-3" size="sm" color="neutral" variant="subtle" icon="i-lucide-refresh-cw" @click="loadMaterials">重试</UButton>
      </div>
      <div v-else-if="materials.length < 2" class="rounded-xl bg-slate-950/40 px-5 py-4">
      ...
      <div v-else class="rounded-xl bg-slate-950/40 px-5 py-4">
    ...
    <!-- 检查结果（主视觉）：每条问题一段——比的是什么、两边各说了什么（左右两列数值与引用）、为什么核对、点引用回原文。 -->
    <section v-if="result" class="mt-8">
      <EmptyState
        v-if="emptyResult"
        title="当前没有发现两份材料对同一指标使用不同数值。"
        :hint="`检查了 ${result.filename_a} 和 ${result.filename_b}；换个对照材料可以再查一次。`"
      />
      <template v-else>
        ...
        <p class="mt-1 text-xs text-slate-500">每一处都是两份材料对同一指标的不同说法。</p>
        ...
        <div class="mt-4 space-y-6">
          ...
          >
            ...
            <!-- 左右对照：主材料在左，对照材料在右，中间 ↔ 连接两边各自的说法。 -->
            <div class="relative mt-4 grid gap-x-10 gap-y-4 sm:grid-cols-2">
              <span
                class="absolute left-1/2 top-1/2 hidden -translate-x-1/2 -translate-y-1/2 font-mono text-xl text-slate-600 sm:block"
                aria-hidden="true"
              >↔</span>
              <div>
                <p class="text-xs text-slate-500">主材料 · {{ result.filename_a }}</p>
                <p v-if="sideValue(finding, 'a')" class="mt-2 font-mono text-3xl font-semibold tracking-tight text-amber-300">
                  {{ sideValue(finding, 'a') }}
                </p>
                <!-- 无数值但有真实引用：用原文摘录当主内容；引用也无法稳定归属时低权重提示，不发明数值。 -->
                <p v-else-if="sideCitations(finding, 'a').length > 0" class="mt-2 text-sm leading-relaxed text-slate-300">
                  “{{ sideCitations(finding, 'a')[0].quote }}”
                </p>
                ...
                <ul class="mt-3 space-y-1">
                  <li v-for="citation in sideCitations(finding, 'a')" :key="`a:${citation.block_id}:${citation.start}`">
                    <button
                      type="button"
                      class="group flex w-full flex-wrap items-baseline gap-x-2 rounded-md px-2 py-1.5 text-left transition hover:bg-slate-800/60"
              ...
              <div>
                <p class="text-xs text-slate-500">对照材料 · {{ result.filename_b }}</p>
                <p v-if="sideValue(finding, 'b')" class="mt-2 font-mono text-3xl font-semibold tracking-tight text-amber-300">
                  {{ sideValue(finding, 'b') }}
                </p>
                <p v-else-if="sideCitations(finding, 'b').length > 0" class="mt-2 text-sm leading-relaxed text-slate-300">
                  “{{ sideCitations(finding, 'b')[0].quote }}”
                </p>
                <p v-else class="mt-2 text-sm text-slate-500">需人工核对</p>
                <ul class="mt-3 space-y-1">
